...
{
    FILE *fp;
    mk_flush_proc flush_proc;
    mk_seek_proc seek_proc;
    void* proc_user_data;
...
mk_writer *mk_create_writer( const char *filename )
{
    ...
    w->flush_proc = NULL;
    w->seek_proc = NULL;
    w->proc_user_data = NULL;

    return w;
}

int mk_set_buffer_writer_internal(mk_writer* w, mk_flush_proc fp, mk_seek_proc sp, void* user_data) {
    w->flush_proc = fp;
    w->seek_proc = sp;
    w->proc_user_data = user_data;
    return 0;
}
...
int mk_close( mk_writer *w, int64_t last_delta )
{
    ...
    if( mk_flush_frame( w ) < 0 || mk_close_cluster( w ) < 0 )
        ret = -1;
    if( w->wrote_header && (!w->fp || x264_is_regular_file( w->fp )) )
    {
        if (!w->fp) {
            if (w->seek_proc) {
                w->seek_proc(w->duration_ptr, w->proc_user_data);
            }
puts("WRITE HEADER");
        } else {
